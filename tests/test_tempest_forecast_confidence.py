"""Confidence wiring for tempest_forecast.py (Pattern E: no combination
step). Its only row (member_id=0) must not change VALUE under this plan;
only confidence may differ. Network access is mocked via a patched _fetch,
matching tests/test_models_contract.py's own pattern for external models."""
from types import SimpleNamespace
from unittest.mock import patch

import models.tempest_forecast as tf_mod
from tests.conftest import make_obs

_ISSUED_AT = 1_700_000_000  # fixed epoch for determinism
_LEAD = 6
_VARIABLE = "temperature"
_CONF = SimpleNamespace(tempest_station_id="99999", tempest_token="fake-token")
...
def _run(**kwargs):
    ...
    with patch.object(tf_mod, "_fetch", return_value=_CANNED):
        return tf_mod.run(obs, _ISSUED_AT, conf=_CONF, **kwargs)
