...
#   7  24h-amp-damped   24h window, temperature amplitude adjustment damped —
#                       scored best on dewpoint but worst on temperature during the
#                       2026-09-12 dry-intrusion event (overcorrected the diurnal
...
_TEMP_DAMP_FACTOR = 0.4    # member 7: fraction of amp_adj applied to temperature

# members 1-3: amplitude only; members 4-6: amplitude + pressure ridge boost;
# member 7: amplitude only, temperature side damped (see module docstring)
_MEMBERS = [
    (1, 24, False, 1.0),
    (2, 48, False, 1.0),
    (3, 72, False, 1.0),
...
def run(obs, issued_at: int, *, conn_in, conn_out=None, weights=None, member_history=None,
    ...
    for _, window_hours, _, _ in _MEMBERS:
    ...
    for lead in LEAD_HOURS:
        ...
        for mid, window_hours, use_pressure, temp_damp in _MEMBERS:
            ...
            amp_adj = dd_anom * _AMP_SENSITIVITY * dev * temp_damp
